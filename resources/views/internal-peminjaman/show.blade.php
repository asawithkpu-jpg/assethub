    ...
    <div class="bg-white dark:bg-darkCard rounded-lg shadow-sm border dark:border-gray-700 p-6">
        <div class="flex justify-between items-center border-b dark:border-gray-800 pb-4 mb-6">
            ...
            <span class="px-2.5 py-1 rounded text-[9px] font-bold uppercase tracking-wider 
                {{ $peminjaman->status == 'Menunggu acc Kasubbag' || $peminjaman->status == 'Menunggu acc Pimpinan' ? 'bg-orange-100 text-orange-600' : ($peminjaman->status == 'Ditolak' ? 'bg-red-100 text-red-600' : ($peminjaman->status == 'Batal' ? 'bg-gray-100 text-gray-600' : 'bg-green-100 text-green-600')) }}">
        ...
        <div class="flex flex-wrap justify-end gap-2 pt-6 border-t dark:border-gray-800">
            @if($peminjaman->status === 'Menunggu acc Pimpinan')
                @can('persetujuan-pimpinan')
                    ...
                    <form action="{{ route('peminjaman-internal.approvePimpinan', $peminjaman->id) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors shadow-sm">Setujui</button>
                    </form>
                @endcan
            @endif
