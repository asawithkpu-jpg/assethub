...
<div class="w-full">
                ...
                <ol class="inline-flex items-center space-x-1 text-[10px] font-medium uppercase tracking-wider">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="font-bold uppercase text-gray-500">peminjaman eksternal</span>
                        </div>
                    </li>
                    <li>
                        <div class="flex items-center">
                            <svg class="w-3 h-3 text-gray-400 mx-1" fill="currentColor" viewBox="0 0 20 20"><path d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z"></path></svg>
                            <span class="text-primary font-bold uppercase decoration-2 underline-offset-4 tracking-widest">proses pengembalian</span>
                        </div>
                    </li>
                </ol>
            </nav>
    ...
    @if ($errors->any())
    <div class="mb-4 p-3 bg-red-100 border-l-4 border-red-500 text-red-700 text-[10px] uppercase font-bold">
        <ul>
    ...
    <div class="bg-white dark:bg-darkCard rounded-lg shadow-sm border dark:border-gray-700 p-6">
                        ...
                        <td class="px-4 py-3 text-center">
                            @if($detail->foto_kembali)
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ asset('storage/eksternal/foto/' . $detail->foto_kembali) }}" target="_blank" class="text-primary hover:underline font-bold text-[9px] uppercase tracking-tighter">Lihat Foto</a>
                                    <button type="button" onclick="confirmDeleteFotoKembali('{{ $detail->id }}')" class="text-red-500 hover:text-red-700 transition">
                            ...
                            @else
                                <form action="{{ route('peminjaman-eksternal.upload-foto-kembali', $detail->id) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <label class="cursor-pointer bg-primary hover:bg-primary/90 text-white text-[9px] font-bold px-2.5 py-1 rounded transition uppercase shadow-sm inline-block">
                                        Unggah
        ...
        {{-- Form Actions --}}
        <form action="{{ route('peminjaman-eksternal.process-return', $peminjaman->id) }}" method="POST" id="formKembali">
            @csrf
            <div class="flex justify-end gap-2 pt-6 border-t dark:border-gray-800">
                <a href="{{ route('peminjaman-eksternal.show', $peminjaman->id) }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-1.5 rounded text-[10px] font-bold uppercase tracking-widest transition">Batal</a>
                <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-1.5 rounded text-[10px] font-bold uppercase tracking-widest transition shadow-md shadow-green-500/20">Simpan Pengembalian</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function confirmDeleteFotoKembali(detailId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                form.method = 'POST';
                form.action = `/peminjaman-eksternal/delete-foto-kembali/${detailId}`;
    ...
    document.getElementById('formKembali').onsubmit = function(e) {
        ...
        Swal.fire({
            ...
            text: "Asset akan dikembalikan ke stok gudang dan status akan berubah menjadi Selesai.",
            icon: 'question',
            showCancelButton: true,
